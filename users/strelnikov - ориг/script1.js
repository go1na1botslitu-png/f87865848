...
function initMemeGenerator() {
	...
	function createCanvasFromMeme(element) {
		html2canvas(element, {
			backgroundColor: null,
			scale: 2
		}).then(canvas => {
			const link = document.createElement('a');
			link.download = 'новогодний-мем.png';
			link.href = canvas.toDataURL('image/png');
			link.click();
			showMemeNotification('Мем успешно скачан!');
		}).catch(error => {
			console.error('Ошибка создания мема:', error);
			showMemeNotification('Ошибка при создании мема');
		});
	}
