...
function initQuiz() {
	...
	const roles = {
		...
		'party-salad-sparkler': {
			...
			icon: '🥗🔥',
			description: 'Ты устраиваешь световое шоу с салатом и бенгальскими огнями. Твоя миссия — чтобы салат пульсировал в такт музыке. Миксовать майонез как диджей.',
			cringe: 85,
			spirit: 94
		},
		...
		'party-hug-tree': {
			...
			icon: '🎄🕺',
			description: 'Ты танцуешь, обнимая ёлку и всех гостей. Твоя миссия — чтобы все обнимались в такт музыке. Танцевать с гирляндой вокруг шеи.',
			cringe: 80,
			spirit: 95
		},
		'party-hug-sparkler': {
...
// В конец файла добавляем функцию для инициализации аудиоплеера
function initAudioPlayer() {
	const audioPlayer = document.getElementById('author-audio');
	...
	// Автоматическое переключение кнопок при окончании трека
	...
	const authorBlock = document.querySelector('.author-message-container');
	authorBlock.style.opacity = '0';
	authorBlock.style.transform = 'translateY(30px)';
	authorBlock.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
...
// Обновляем функцию DOMContentLoaded для инициализации аудиоплеера
document.addEventListener('DOMContentLoaded', function() {
	createSnowflakes();
	...
	initQuiz();
	initMemeGenerator();
	initAudioPlayer(); // Добавляем инициализацию аудиоплеера
});
	}
}