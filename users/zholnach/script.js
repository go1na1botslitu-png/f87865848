...
function initQuiz() {
    ...
    const roles = {
        ...
        'relax-tv-champagne': {
            ...
            icon: '😴🍾',
            description: 'Ты смотришь телевизор, принимая шампанское анально. Твоя миссия — устроить алкогольное сквирт на пульт. Открывать бутылки анусом.',
            cringe: 97,
            spirit: 68
        },
        'relax-tv-tangerine': {
...
function initVideoMemories() {
    ...
    memoriesContainer.style.transform = 'translateY(30px)';
    memoriesContainer.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
    
    setTimeout(() => {
        memoriesContainer.style.opacity = '1';
        memoriesContainer.style.transform = 'translateY(0)';
    }, 400);
    
    // Простая инициализация видео - они будут работать с нативными контролами
...
function initAuthorMessage() {
    const authorMessageContainer = document.querySelector('.author-message-container');
    if (!authorMessageContainer) return;
    
    authorMessageContainer.style.opacity = '0';
    authorMessageContainer.style.transform = 'translateY(30px)';
    authorMessageContainer.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
    
    setTimeout(() => {
        authorMessageContainer.style.opacity = '1';
        authorMessageContainer.style.transform = 'translateY(0)';
    }, 500);
}